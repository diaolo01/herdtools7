...
let pp = function
  | Addr s -> s
  | Pte s -> Misc.pp_pte s
  | Ttd { stage = Stage1; level = Lv3; s } -> Misc.pp_pte s
  | Ttd { stage = Stage1; level = Lv2; s } -> Misc.pp_ttd "S1" "LV2" s
  | Ttd _ -> assert false
...
let tr_symbol =
  ...
  function
    ...
    | System (TTD {stage; level},s) ->
      Ttd {
        stage = litmus_stage_of_herd stage;
        level = litmus_level_of_herd level;
        s; }
