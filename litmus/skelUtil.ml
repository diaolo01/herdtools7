...
module BlockValUtil(B:BlockVal.S) = struct
  let dump_blockval_flags s p =
    match B.as_flags p with
    | Some msk -> sprintf "litmus_set_pmd_flags(%s,%s)" s msk
    | None -> s
end

module TableValUtil(T:TableVal.S) = struct
  let dump_tableval_flags s p =
    match T.as_flags p with
    | Some msk -> sprintf "litmus_set_pmd_flags(%s,%s)" s msk
    | None -> s
end
